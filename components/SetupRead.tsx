    ...
    <Panel
      ...
      action={
        <button
          ...
          onClick={ask}
          disabled={loading}
          className="rounded border border-[var(--color-border)] px-2 py-1 text-micro text-[var(--color-muted)] transition-colors hover:text-[var(--color-text)] disabled:opacity-50"
        >
          {loading ? 'Thinking…' : result?.available ? 'Ask again' : 'Ask the model'}
        </button>
      }
    >
      <p className="px-4 py-3 text-micro leading-relaxed text-[var(--color-muted)]">{brief}</p>
          ...
          {result.available ? (
            <>
              <div className="mb-1.5 flex items-center gap-2">
                <span className="rounded bg-[var(--color-surface-2)] px-1.5 py-0.5 font-mono text-micro text-[var(--color-muted)]">
                  AI · {result.model}
                </span>
              ...
              <p className="text-micro leading-relaxed whitespace-pre-line">{result.text}</p>
            </>
          ) : (
