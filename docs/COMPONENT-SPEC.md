...
| Crowd | retail feed first (Myfxbook, then OANDA position book — whichever is configured), else CFTC non-reportable per symbol | retail long share | contrarian band | ≥60% long −1, ≤40% +1 | latest snapshot / report | 20 min (feed) or weekly (CFTC) | −1…+1 | **crosses null without a feed** — never synthesised; metals stay on CFTC | CONFIRMED transformation; feed agreement with A1 UNMEASURED | `a1-crowd-is-fxssi` 8/8; `crowd:no-public-cross-feed-passes-the-gate`; `npm run check:oanda` |
| Rates (FX) | A1 projections snapshot while current; once any bank decides after it (or it ages past 31 days), the TradingView central-bank decision calendar for all eight banks at once | next decision's consensus vs standing rate | base − quote of each leg's direction | 0.1pp | next scheduled decision | hourly (calendar) | −1…+1 per leg | one bank unreadable → whole board falls back | CONFIRMED | `a1-rates-is-policy-vs-projection` 8/8; `rates:calendar-consensus-supersedes-stale-snapshot` 09-15 + 09-17 |
